...
{
    /// <summary>
    /// One shared frame loop per <see cref="TopLevel"/> for the whole motion layer — the
    /// proven engine of SukiUI.ControlsAnimation, moved as-is. All animated controls
    /// register a callback here; exactly one callback fires per rendered frame while at
    /// least one subscriber is active, and zero when nothing animates.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Subscribe against the TopLevel of the VISUAL being animated. A frame only renders
    /// when something invalidates: subscribers must produce at least one property write
    /// (even a no-op-looking initial pose) to schedule the frame their first callback will
    /// ride on — the channels do this by advancing once synchronously at subscription time.
    /// </para>
    /// <para>
    ...
    public static class SukiTicker
    {
        ...
        // const, so the fallback branch below stays reachable-warning-free.)
        private static readonly bool UseRequestAnimationFrame = true;
